 ...
 * 展示完整的工具链检测和配置生成流程
 */
async function demonstrateArmToolchainDetection(): Promise<void> {
    ...
    try {
        ...
        const toolchainInfo = await getArmToolchainInfo(toolchainPath);
        ...
        const executables = getArmToolchainExecutables(toolchainInfo.rootPath);
        ...
        const debugConfig = await generateCortexDebugConfig(toolchainPath, {
        ...
        console.log(`   配置名称: ${(debugConfig as any).name}`);
        console.log(`   目标设备: ${(debugConfig as any).device}`);
        console.log(`   调试器路径: ${(debugConfig as any).debuggerPath}`);
        console.log();

        // 6. 生成launch.json内容
        console.log('6. 生成launch.json文件内容...');
        
        const launchJsonContent = await generateLaunchJsonContent(toolchainPath, [
            {
        ...
        console.log(`   VS Code版本: ${(launchJsonContent as any).version}`);
        console.log(`   配置数量: ${(launchJsonContent as any).configurations.length} 个`);
        
        // 显示生成的配置摘要
        (launchJsonContent as any).configurations.forEach((config: any, index: number) => {
 ...
 * 演示类型定义和接口使用
 */
function demonstrateTypeDefinitions(): void {
    ...
    const debugConfig: CortexDebugConfig = {
...
// 导出演示函数供其他模块使用
export {
    demonstrateArmToolchainDetection,
    demonstrateTypeDefinitions,
    runDemo
};