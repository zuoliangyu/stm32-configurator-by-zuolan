...
import { ToolchainDetectionResults, DetectionStatus } from '../ui/types';
import { 
    DetectionOptions, 
    ExtendedToolchainDetectionResults 
} from './detectionTypes';
import { DefaultCacheManager } from './cacheManager';
import { DetectorFactory } from './toolchainDetectors';
 ...
 * @since 0.2.5
 */
export class ToolchainDetectionService {
    /** 单例实例 */
    private static instance: ToolchainDetectionService;
    
    /** 缓存管理器 */
    private cacheManager: DefaultCacheManager;
    
    /** 默认缓存有效期（5分钟） */
    private readonly DEFAULT_CACHE_VALIDITY_MS = 5 * 60 * 1000;
     ...
     * 获取服务实例（单例模式）
     */
    public static getInstance(): ToolchainDetectionService {
     ...
     * 检测所有工具链
     */
    public async detectToolchains(options: DetectionOptions = {}): Promise<ExtendedToolchainDetectionResults> {
        const {
            forceRedetection = false,
            specificTools,
            cacheValidityMs = this.DEFAULT_CACHE_VALIDITY_MS
        } = options;
     ...
     * 获取缓存的检测结果
     */
    public getCachedResults(): ExtendedToolchainDetectionResults | null {
        return this.cacheManager.getCached();
    }
     ...
     * 转换为UI兼容的检测结果格式
     */
    public toUICompatibleResults(results: ExtendedToolchainDetectionResults): ToolchainDetectionResults {
        return {
            openocd: {
     ...
     * 使用多种检测策略和启发式算法找到工具链
     */
    public async performIntelligentDetection(): Promise<{
        results: ExtendedToolchainDetectionResults;
        insights: {
            detectionStrategy: string;
            alternativeLocations: string[];
            installationRecommendations: string[];
        ...
        const insights = {
            ...
            installationRecommendations: [] as string[],
        ...
        // 2. 如果标准检测失败，执行高级检测
        if (results.openocd.status !== DetectionStatus.SUCCESS) {
            await this.performAdvancedOpenOCDDetection(results, insights);
        }

        if (results.armToolchain.status !== DetectionStatus.SUCCESS) {
     ...
     * 检查工具链是否完整且功能正常
     */
    public async validateToolchainIntegrity(toolchainPath: string, type: 'openocd' | 'arm'): Promise<{
        isValid: boolean;
        completeness: number;
     ...
     * 尝试自动解决常见的配置问题
     */
    public async autoRepairConfiguration(): Promise<{
        ...
        newDetectionResults?: ExtendedToolchainDetectionResults;
    }> {
        const repair = {
            repairsAttempted: 0,
            repairsSuccessful: 0,
            repairActions: [] as Array<{
                action: string;
                target: string;
                result: 'success' | 'failed' | 'skipped';
                details: string;
            }>,
            newDetectionResults: undefined as ExtendedToolchainDetectionResults | undefined
        };
    ...
    private async performDetection(
    ...
    ): Promise<ExtendedToolchainDetectionResults> {
        ...
        if (!specificTools || specificTools.includes('openocd')) {
            tasks.push(DetectorFactory.getOpenOCDDetector().detect(detectionTime)
                .then(result => { results.openocd = result; }));
        }
        if (!specificTools || specificTools.includes('armToolchain')) {
            tasks.push(DetectorFactory.getArmToolchainDetector().detect(detectionTime)
     ...
     * @private
     */
    private createEmptyResults(): ExtendedToolchainDetectionResults {
     ...
     * 执行高级OpenOCD检测
     */
    private async performAdvancedOpenOCDDetection(
        results: ExtendedToolchainDetectionResults, 
        insights: any
    ): Promise<void> {
        const commonPaths = [
        ...
        const customPath = config.get<string>('openocdPath');
     ...
     * 执行高级ARM工具链检测
     */
    private async performAdvancedArmToolchainDetection(
        results: ExtendedToolchainDetectionResults, 
        insights: any
    ): Promise<void> {
        const commonPaths = [
     ...
     * 检查兼容性问题
     */
    private async checkCompatibilityIssues(
        results: ExtendedToolchainDetectionResults, 
        insights: any
    ): Promise<void> {
        // 检查版本兼容性
     ...
     * 验证OpenOCD完整性
     */
    private async validateOpenOCDIntegrity(toolchainPath: string, validation: any): Promise<any> {
     ...
     * 验证ARM工具链完整性
     */
    private async validateArmToolchainIntegrity(toolchainPath: string, validation: any): Promise<any> {
        const requiredTools = [
            'arm-none-eabi-gcc',
            'arm-none-eabi-g++',
            'arm-none-eabi-gdb',
     ...
     * 查找ARM工具链bin路径
     */
    private findArmToolchainBinPath(basePath: string): string | null {
        const possiblePaths = [
     ...
     * 查找版本目录
     */
    private findVersionDirectories(basePath: string): string[] {
        try {
     ...
     * 检查执行权限
     */
    private checkExecutePermission(filePath: string): boolean {
        try {
     ...
     * 修复PATH环境变量
     */
    private async repairPathEnvironment(repair: any): Promise<void> {
     ...
     * 修复VS Code设置
     */
    private async repairVSCodeSettings(repair: any): Promise<void> {
        repair.repairsAttempted++;
     ...
     * 修复权限问题
     */
    private async repairPermissionIssues(repair: any): Promise<void> {
        repair.repairsAttempted++;
     ...
     * 查找OpenOCD路径的辅助方法
     */
    private async findOpenOCDPath(): Promise<string | null> {
