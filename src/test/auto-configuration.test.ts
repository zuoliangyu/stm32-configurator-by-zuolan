...
import { AutoConfigurationService } from '../services/autoConfigurationService';
import { ConfigurationScanner } from '../services/configurationScanner';
import { CortexDebugConfigGenerator } from '../services/cortexDebugConfigGenerator';
...
suite('Auto-Configuration Tests', () => {
    ...
    suite('AutoConfigurationService', () => {
        test('should create singleton instance', () => {
            const service1 = AutoConfigurationService.getInstance();
            const service2 = AutoConfigurationService.getInstance();
            
            assert.strictEqual(service1, service2, 'Should return same instance');
        });

        test('should scan configuration successfully', async () => {
            const service = AutoConfigurationService.getInstance();
    ...
    suite('CortexDebugConfigGenerator', () => {
        test('should generate basic configuration', () => {
            const generator = new CortexDebugConfigGenerator();
    ...
    suite('ToolchainDetectionService', () => {
        ...
        test('should detect toolchains', async () => {
            ...
            ['openocd', 'armToolchain'].forEach(tool => {
                const toolResult = results[tool as keyof Omit<typeof results, 'completedAt'>];
                if (typeof toolResult !== 'number') {
                    assert.ok(typeof toolResult.name === 'string', `${tool} should have name`);
                    assert.ok([DetectionStatus.SUCCESS, DetectionStatus.FAILED].includes(toolResult.status), `${tool} should have valid status`);
    ...
    suite('AutoConfigurationDialog', () => {
        test('should create dialog instance', () => {
            const dialog = new AutoConfigurationDialog(mockContext);
    ...
    suite('Integration Tests', () => {
        test('should complete end-to-end auto-configuration flow', async () => {
            ...
            const autoConfigService = AutoConfigurationService.getInstance();
