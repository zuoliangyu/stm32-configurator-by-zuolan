...
const detectionProgressHandler_1 = require("../ui/detectionProgressHandler");
const resultDisplayHandler_1 = require("../ui/resultDisplayHandler");
const userInteractionHandler_1 = require("../ui/userInteractionHandler");
...
describe('UI Components Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockLocalizationManager = {
            getString: sandbox.stub().callsFake((key) => {
                const strings = {
            ...
            formatString: sandbox.stub().callsFake((template, ...args) => {
    ...
    describe('DetectionProgressHandler Tests', () => {
        let progressHandler;
        beforeEach(() => {
            progressHandler = new detectionProgressHandler_1.DetectionProgressHandler(mockLocalizationManager);
        });
    ...
    describe('ResultDisplayHandler Tests', () => {
        let displayHandler;
        beforeEach(() => {
            displayHandler = new resultDisplayHandler_1.ResultDisplayHandler(mockLocalizationManager);
        });
        ...
        it('should show results dialog for successful detection', async () => {
            ...
            assert.strictEqual(result, types_1.UserAction.CONTINUE);
        });
        it('should show results dialog for failed detection', async () => {
            ...
            const showInformationMessageStub = sandbox.stub(vscode.window, 'showInformationMessage')
                .resolves('Configure Manually');
            const result = await displayHandler.showResultsDialog(mockResults);
            assert.ok(showInformationMessageStub.called);
            assert.strictEqual(result, types_1.UserAction.CONFIGURE_MANUALLY);
        });
        it('should handle user cancellation in results dialog', async () => {
            ...
            const result = await displayHandler.showResultsDialog(mockResults);
            assert.strictEqual(result, types_1.UserAction.CANCEL);
        });
    ...
    describe('UserInteractionHandler Tests', () => {
        ...
        beforeEach(() => {
            interactionHandler = new userInteractionHandler_1.UserInteractionHandler(mockLocalizationManager);
        });
    ...
    describe('ConfigurationHandler Tests', () => {
        ...
        it('should save toolchain configuration successfully', async () => {
            ...
            const openocdCall = updateStub.getCalls().find(call => call.args[0] === 'openocdPath');
